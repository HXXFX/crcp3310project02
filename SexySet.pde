class SexySet  //<>//
{
  ...
  float xOffset;
  //ArrayList<Food> setData = new ArrayList<Food>(10);




  public SexySet(PVector initialLocation)  //constr intoalize var ... initiallocation is var pass in 
  {
  ...
  Set<Integer> dinnerColor = new HashSet<Integer>();
  void eat(Food f)
  {
    boolean iWantToEat = false;
    Food x = f;
    int y =  x.foodColor;
    if (dinnerColor.add(y))
    {
      iWantToEat = true;
      println( "set is eatting");
    }
